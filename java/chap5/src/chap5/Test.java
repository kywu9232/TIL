...
class Employ {
 	...
 	Employ(int id, String name, int basePay, int genralFee){
 		this.id = id;
 		this.name = name;
...
class Manag extends Employ{
	int manFee;
	Manag(int id, String name, int basePay, int generalFee, int manFee){
...
class Secretary extends Employ {
	int secFee;
...
public static void main(String args[]){
	...
	// 1.상속 2.생성자(타입 순서 갯수....)
	e[0] = new Employ(1000,"이사원",10000,5000);
	e[1] = new Manag(2000,"김간부",20000,10000,10000);
 ...
 */ 
	...
	for(int i = 0; i < e.length; i++) {
		...
		if(e[i] instanceof Manag) { // i=0,1,2(false) i=3(true)
			System.out.println( i + "번째 객체=" +  ((Manag)e[i]).manFee    );
		}//if
	}//for
	}//main
}//class